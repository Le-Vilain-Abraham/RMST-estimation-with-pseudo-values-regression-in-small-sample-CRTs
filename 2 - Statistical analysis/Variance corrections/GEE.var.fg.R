##########################################################################################################################################################
# Fay and Graubard variance correction
##########################################################################################################################################################

# Function from the geesvm package modified to be used for CRTs with variable cluster sizes
...
# corstr: working correlation matrix ("independence" for the independent correlation matrix or "exchangeable" for the exchangeable correlation matrix)
# b: user-specified constant (see Fay and Graubard 2001)
...
GEE.var.fg <-
  function(formula, id, family = gaussian, data, corstr="independence", b = 0.75){
    ...
    for (i in 1:size){
      ...
      if (family=="gaussian"){
        #FLVA November 2022: replacing "var" by "var1" for CRTs with variable cluster sizes
